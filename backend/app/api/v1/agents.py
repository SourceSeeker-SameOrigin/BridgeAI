...
async def get_agent(
    ...
    tenant_id = str(current_user.tenant_id) if current_user.tenant_id else None
    agent = await agent_service.get_agent(agent_id, db, tenant_id=tenant_id)
    return ApiResponse.success(data=agent)
...
async def update_agent(
    ...
    tenant_id = str(current_user.tenant_id) if current_user.tenant_id else None
    agent = await agent_service.update_agent(agent_id, request, db, tenant_id=tenant_id)
    return ApiResponse.success(data=agent)
...
async def delete_agent(
    ...
    tenant_id = str(current_user.tenant_id) if current_user.tenant_id else None
    await agent_service.delete_agent(agent_id, db, tenant_id=tenant_id)
