...
from app.core.tenant_filter import set_current_tenant
...
class TenantMiddleware(BaseHTTPMiddleware):
    ...
    3. Set tenant_id in async ContextVar for SQLAlchemy safety-net filter
    4. For authenticated requests, the tenant_id from JWT/user takes priority
       (enforced by get_current_user -> user.tenant_id)
    5. Reject requests where X-Tenant-Id header doesn't match the authenticated
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        # Set tenant context for SQLAlchemy safety-net listener
        set_current_tenant(header_tenant_id)
