...
async def _execute_node(
    ...
    try:
        if node_type in ("llm_call", "llm"):
            ...
            output = f"[Loop] Would iterate up to {max_iterations} times"
            return WorkflowStepResult(
                node_id=node_id,
                node_type=node_type,
                status="success",
            ...
            output = f"[Parallel] Would execute {len(branches)} branches"
            return WorkflowStepResult(
                node_id=node_id,
                node_type=node_type,
                status="success",
...
async def execute_workflow(
    ...
    while queue:
        ...
        result = await _execute_node(node, input_text, variables, last_output)
        steps.append(result)
