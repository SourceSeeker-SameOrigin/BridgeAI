...
export async function createOrder(data: CreateOrderRequest): Promise<CreateOrderResponse> {
  const res = await client.post<CreateOrderResponse>('/payment/orders', data)
  return res.data
}
...
export async function getOrders(): Promise<PaymentOrder[]> {
  const res = await client.get<PaymentOrder[] | { items: PaymentOrder[] }>('/payment/orders')
  // Backend may return array or paginated response
  const data = res.data
  return Array.isArray(data) ? data : (data as { items: PaymentOrder[] }).items || []
}
