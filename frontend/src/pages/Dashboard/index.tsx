...
export default function DashboardPage() {
  ...
  const cards = [
    ...
    {
      title: t('dashboard.tokenUsage', 'Token 用量'),
      value: (stats?.daily_usage ?? []).reduce((sum, d) => sum + (d.tokens || 0), 0),
      suffix: 'tokens',
      icon: <DollarOutlined />,
      color: '#f97316',
    },
    {
      title: t('dashboard.avgResponseTime', '平均响应时间'),
      value: '-',
      suffix: '',
        ...
        <Col span={24}>
          <GlassCard hoverable={false}>
            <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 20, color: '#e2e8f0' }}>
              ...
              {t('dashboard.recentConversations', '最近对话')}
            </div>
            <Table
              ...
              pagination={false}
              size="small"
              locale={{ emptyText: t('dashboard.noConversations', '暂无对话记录') }}
              columns={[
                {
                  title: t('dashboard.conversationTitle', '对话标题'),
                  dataIndex: 'title',
                  key: 'title',
                  render: (title: string) => (
                    <span style={{ color: '#e2e8f0' }}>{title || t('chat.newConversationTitle', '新对话')}</span>
                  ),
                },
                {
                  title: t('dashboard.agentName', 'Agent'),
                  dataIndex: 'agentName',
                  key: 'agentName',
                  width: 160,
                  render: (name: string) => (
                ...
                {
                  title: t('dashboard.messageCountCol', '消息数'),
                  dataIndex: 'messageCount',
                  key: 'messageCount',
                  width: 100,
                  render: (count: number) => (
                    <span style={{ color: '#94a3b8' }}>{count ?? 0}</span>
                  ),
                },
                {
                  title: t('dashboard.time', '时间'),
                  dataIndex: 'updatedAt',
                  key: 'updatedAt',
                  width: 180,
                  render: (time: string) => (
