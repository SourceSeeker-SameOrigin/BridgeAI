...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    // Also load MCP connector names for display
    getMcpConnectors().then((connectors) => {
      const names: Record<string, string> = {}
      connectors.forEach((c) => { names[c.id] = c.name })
                  ...
                  <div style={{ borderTop: '1px solid rgba(148,163,184,0.1)', paddingTop: 16 }}>
                    ...
                    {(currentAgent.tools || []).length > 0 ? (
                      currentAgent.tools.map((tool) => (
                        ...
                        >
                          {connectorNames[String(tool)] || String(tool).substring(0, 8) + '...'}
                        </Tag>
                      ))
                    ) : (
