...
┌─────────────────────────────────────────────────────────────┐
│                      接入层 (Channels)                       │
│   企微机器人 │ 钉钉机器人 │ 飞书机器人 │ Web Chat │ API      │
├─────────────────────────────────────────────────────────────┤
│                  6 阶段对话处理管线 (Pipeline)                 │
│  ① 请求接入 → ② Prompt融合(含分析指令)                      │
│  → ③ 智能模型路由 → ④ AI调用(熔断降级)                      │
...
│   ┌─────────────┐  ┌──────────────┐  ┌───────────────┐      │
│   │ 对话管理器   │  │ 3级记忆管理器 │  │ 权限管理器     │      │
│   │ (会话/历史)  │  │ (工作/短/长)  │  │ (RBAC/审计)   │      │
│   └─────────────┘  └──────────────┘  └───────────────┘      │
├─────────────────────────────────────────────────────────────┤
│              上下文分析 (由大模型一次性输出)                    │
│   情绪/意图/复杂度/关键事实 → 模型回复时同时输出 JSON 分析     │
│   无需规则引擎，零维护，一次调用完成回答+分析                  │
├─────────────────────────────────────────────────────────────┤
│                    能力层 (Capabilities)                      │
│   ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐   │
│   │ RAG 引擎  │  │ MCP 网关  │  │ 行业插件  │  │ 工具集   │   │
│   │ 知识库检索 │  │ 系统连接  │  │ 垂直能力  │  │ 内置工具 │   │
│   └──────────┘  └──────────┘  └──────────┘  └──────────┘   │
├─────────────────────────────────────────────────────────────┤
│                 模型适配层 (Model Router)                      │
│   5层决策: 意图选型→复杂度→用户等级→性能→成本                  │
│   熔断降级链: Claude → DeepSeek → Qwen → Ollama 本地         │
├─────────────────────────────────────────────────────────────┤
│                   基础设施层 (Infrastructure)                  │
│   PostgreSQL+Pgvector │ Redis(3级缓存) │ Celery │ MinIO/OSS  │
│   4维多租户隔离: tenant × user × agent × session             │
└─────────────────────────────────────────────────────────────┘
```
...
CREATE TABLE agent_memories (
    ...
    embedding vector(1536),          -- 语义向量（用于相关性检索）
    metadata JSONB DEFAULT '{}',
